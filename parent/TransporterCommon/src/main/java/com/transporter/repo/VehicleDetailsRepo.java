...
public interface VehicleDetailsRepo extends JpaRepository<VehicleDetails, Integer>{
	...
	@Query(value = "select *, ( 3959 * ACOS( COS( RADIANS(:lattitude) ) * COS( RADIANS( vd.current_lattitude ) ) \r\n"
			...
			+ "FROM vehicledetails vd INNER JOIN driverdetails dr ON (vd.driver_id = dr.id AND dr.driver_verification_status = 'completed' AND dr.riding_status != 1 AND dr.onRoad = 1) WHERE vd.vehicle_type = :vehicleType AND  vd.vehicle_verification_status=1\r\n"
